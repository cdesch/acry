...
const createPosts = () => {
  const  query  =  `
       INSERT INTO posts (author, createDate, title, description) VALUES ('Larry', 'Amazing Work', 'awesome', 'lorem');
`;
  return database.run(query);
}
...
const createPosts2 = () => {
  const  query  =  `
       INSERT INTO posts (author, createDate, title, description) VALUES ('Sarah', 'ACME Scandal', 'terrible book', 'lorem');
`;
  // INSERT INTO posts (author, createDate, title, description) VALUES ('James', 'Novel of the Decade', 'Best Book','lorem');
  return database.run(query);
}
